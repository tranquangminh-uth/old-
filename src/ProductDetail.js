...
const ProductDetail = ({ product, onBack }) => {
  ...
  // Giải mã token để biết ai đang đăng nhập
  ...
  // Nếu ID người đăng nhập = ID chủ sản phẩm -> Hiện nút Xóa
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.location.reload(); // Xóa xong tải lại trang chủ
      } else {
  ...
  return (
    ...
    >
      {/* THANH CÔNG CỤ TRÊN CÙNG: Nút Quay Lại + Nút Xóa  */}
      <div
        style={{
          display: "flex",
        ...
        <div style={{ flex: "1 1 400px" }}>
          ...
          {showOrderForm ? (
            ...
            <>
              ...
              {/* NẾU LÀ CHỦ SẢN PHẨM THÌ KHÔNG CẦN HIỆN NÚT MUA VÀ CHAT */}
              {!isOwner && (
                <div
